...
#ifdef _WIN32
#include <windows.h>
typedef HANDLE vt_handle;  /* Windows terminal handle */
#else
#include <termios.h>
#endif
...
typedef struct vt_tag {                 /* virtual console type */
    ...
    int blind;                          /* blind input mode */
#ifdef _WIN32
    vt_handle handle;                   /* Windows terminal handle */
#else
    struct termios tio;                 /* POSIX terminal attribute */
#endif
