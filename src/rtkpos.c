...
/* global variables add by Liuchunbo ----------------------------------------------------------*/
static int satpos_flag = 1;
...
extern int rtkoutstat(rtk_t *rtk, int level, char *buff)
{
    ...
    time2str(rtk->sol.time, sol_time_str, 3);
    ...
    dops(valid_sat_count,azel,0.0,dop);
    p += sprintf(p, "$DOP,%s,%.2f,%.2f,%.2f,%.2f\n", sol_time_str, dop[0], dop[1], dop[2], dop[3]);
                 ...
                 double *y, double *e, double *azel, double *freq)
{
    ...
    for (i=0;i<n;i++) {
		...
		antmodel(obs[i].sat,opt->pcvr+base,opt->antdel[base],azel+i*2,opt->posopt[1],
                 dant);
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    if(obs[0].time.time== temp_a.time && obs[0].time.sec==temp_a.sec){
        // char buff_temp[64] = time_str(obs[0].time,3);
        int a = 0;
    }

    trace(2,"rtkpos  : time=%s n=%d\n",time_str(obs[0].time,3),n);
    // trace(22,"obs=\n"); traceobs(4,obs,n);
    // trace(22,"nav=\n"); tracenav(5,nav);
    // trace(22,"B2b=\n"); traceB2b_impl(5,nav);
    ...
    time=rtk->sol.time; /* previous epoch */
     ...
     //satposopt
    /* Add by Liuchunbo@APM for real-time satellite position */
    /* satellite position and clock at obs time */
    int svh_otp[MAXSAT];
    double *rs_otp,*dts_otp,*var_otp;
    rs_otp=mat(6,MAXSAT); dts_otp=mat(2,MAXSAT); var_otp=mat(1,MAXSAT);
    if (!rs_otp || !dts_otp || !var_otp) {
        errmsg(rtk, "memory allocation error\n");
        if (rs_otp) free(rs_otp);
        if (dts_otp) free(dts_otp);
        if (var_otp) free(var_otp);
        return 0;
    }
    if (rtk->opt.B2b_format != 0) {
        snprintf(temp_stationname, 128, "B2b_%s", rtk->opt.sationname);
    } 
    else {
        snprintf(temp_stationname, 128, "WUM_%s", rtk->opt.sationname);
    }

    if(0) satposs_otp(temp_stationname,obs[0].time,nav,opt->sateph,opt->sampling,rs_otp,dts_otp,var_otp,svh_otp);
	...
	else
	{
		...
		for (i = 0; i < 7; i++)
			rtk->sol.dtr[i] = rtk->sol.dtr[i] / CLIGHT;
	}
