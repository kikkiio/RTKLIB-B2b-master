...
#ifdef _WIN32
#define DEF_DEV     "COM1"              /* default console device for Windows */
#else
#define DEF_DEV     "/dev/tty"          /* default console device for POSIX */
#endif
...
extern vt_t *vt_open(int sock, const char *dev)
{
    ...
    if (!sock) {
#ifdef _WIN32
        vt->handle = CreateFile(dev ? dev : DEF_DEV, GENERIC_READ | GENERIC_WRITE,
                                0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
        if (vt->handle == INVALID_HANDLE_VALUE) {
            free(vt);
            return NULL;
        }
        vt->in = vt->out = (int)vt->handle; /* Cast for compatibility */
#else
...
#ifdef _WIN32
    DWORD written;
    return WriteFile((HANDLE)vt->out, buff, p - buff, &written, NULL) && written == (DWORD)(p - buff);
#else
...
#ifdef _WIN32
    DWORD written;
    return WriteFile((HANDLE)vt->out, vt->buff + vt->cur, 1, &written, NULL) && written == 1;
#else
    return write(vt->out, vt->buff + vt->cur, 1) == 1;
#endif
    vt->cur++;
    return 1;
}
...
static int left_cur(vt_t *vt)
{
    ...
    char back = '\b';
    DWORD written;
    return WriteFile((HANDLE)vt->out, &back, 1, &written, NULL) && written == 1;
#else
...
#ifdef _WIN32
    DWORD written;
    return WriteFile((HANDLE)vt->out, vt->blind ? "*" : &c, 1, &written, NULL) && written == 1 && ref_buff(vt);
#else
    return write(vt->out, vt->blind ? "*" : &c, 1) == 1 && ref_buff(vt);
#endif
}
...
#ifdef _WIN32
        DWORD written;
        return WriteFile((HANDLE)vt->out, msg, 3, &written, NULL) && written == 3;
#else
        return write(vt->out, msg, 3) == 3;
#endif
...
extern int vt_getc(vt_t *vt, char *c)
{
    *c = '\0';
    ...
    SetCommTimeouts(vt->handle, &timeouts);
    DWORD read_len;
    if (!ReadFile(vt->handle, c, 1, &read_len, NULL) || read_len != 1) {
...
#ifdef _WIN32
    DWORD written;
    return WriteFile((HANDLE)vt->out, &c, 1, &written, NULL) && written == 1;
#else
