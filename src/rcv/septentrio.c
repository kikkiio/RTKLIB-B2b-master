...
static int decode_meas3ranges(raw_t *raw) {
    ...
    refEpochInterval = Meas3_EpochIntervals[misc >> 4]; /* interval for full epoche data */
        ...
        (meas3_refEpoch.TOW == (uint32_t)(TOW / refEpochInterval)*refEpochInterval))  // or reference epoch is available
    {
        for (int navsys = 0; navsys < MEAS3_SYS_MAX; navsys++) {
			...
			uint8_t *p_navsys = p + idx, idx_navsys = 0, BF1 = 0, nB = 0, signalIndexMasterShort = 0, signalExcludedPresent = 0;
            ...
            BF1 = U1(p_navsys+idx_navsys);
            nB = BF1 & 0x7;
            signalIndexMasterShort = (BF1 >> 3) & 0xf;
            signalExcludedPresent = (BF1 >> 7) != 0;
            idx_navsys++;
            if (nB == 7) nB = 8;
