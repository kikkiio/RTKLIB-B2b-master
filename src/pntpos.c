                   ...
                   double *azel, int *vsat, double *resp, int *ns)
{
    ...
    double stop_epoch[6] = {2025,3,7,5,54,30};  
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        satno2id(149,id1);
        satno2id(129,id1);
        if (sat >= 106 && sat < 125) continue;     /* skip BDS-2 satellites(C01~C19) */
        
        // if (debug_epoch[3] <2 && sat == 144) continue;     /* skip C39 */
        // if (debug_epoch[3] <2 && sat == 129) continue;     /* skip C24 */
        if (debug_epoch[3] <2 && sat == 32) continue;         /* skip G32 */
