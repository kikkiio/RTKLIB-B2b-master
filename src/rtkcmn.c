...
static char codepris[7][MAXFREQ][16]={  /* code priority for each freq-index */
    ...
    {"C"       ,"IQX"       ,""        ,""       ,""       ,""}, /* SBS */
    {"IQXDPAN" ,"IQXDPZ"    ,"DPX"    ,"PDX"     ,"DPX"    ,"PDX"}, /* BDS */
    {"ABCX"    ,"ABCX"      ,""        ,""       ,""       ,""}  /* IRN */
};
...
static int readantex(const char *file, pcvs_t *pcvs)
{
	...
	while (fgets(buff,sizeof(buff),fp)) {
		...
		if (strstr(buff+60,"START OF ANTENNA")) {
			...
			pcv.var = (double (*)[1600])calloc(NANTFREQ*1600,sizeof(double));
            if (!pcv.var) { fclose(fp); return 0; }
		}
		...
		if (strstr(buff+60,"TYPE / SERIAL NO")) {
			...
			if (csys=='G')
				freq = f==5?ANT_GPS_L5+1:(f==1||f==2?f:0);
			else if (csys=='R')
				freq = f+NFREQ;
			else if (csys=='C'){
//				freq = f+2*NFREQ;
				if (f==1) freq = ANT_B1C+1;
                else if (f==5) freq = ANT_B2A+1;
                else if (f==2) freq = 1+2*NFREQ;
...
extern void freenav(nav_t *nav, int opt)
{
    ...
    if (opt&0x01) {free(nav->gal_eph);nav->gal_eph=NULL;nav->ngal_eph=0;}
...
{
    int j,idx=signal_ant_index(pcv,sys,code,fallback);
    double e[3],off[3];
    *dant=0.0;
    /* Explicitly unspecified receiver + PCV disabled: geometric antenna
       delta remains usable without inventing signal calibration entries. */
    if (!pcv->sat&&!pcv->type[0]&&!pcvopt&&del&&azel) {
        e[0]=sin(azel[0])*cos(azel[1]); e[1]=cos(azel[0])*cos(azel[1]); e[2]=sin(azel[1]);
        *dant=-dot(del,e,3);return 1;
    }
    if (idx<0||!pcv->var) return 0;
    if (pcv->sat) {
        if (pcvopt) *dant=interpvar0(pcv->sat,nadir*R2D*5.0,pcv->var[idx]);
    }
    else {
        e[0]=sin(azel[0])*cos(azel[1]); e[1]=cos(azel[0])*cos(azel[1]); e[2]=sin(azel[1]);
        for (j=0;j<3;j++) off[j]=pcv->off[idx][j]+del[j];
        *dant=-dot(off,e,3);
        if (pcvopt) *dant+=pcv->dazi!=0.0?
            interpvar1(azel[0]*R2D,90.0-azel[1]*R2D,(pcv_t *)pcv,idx):
            interpvar0(0,90.0-azel[1]*R2D,pcv->var[idx]);
    }
    return 1;
}
...
{
    int f,j,idx;
    double e[3]={sin(azel[0])*cos(azel[1]),cos(azel[0])*cos(azel[1]),sin(azel[1])};
    double off[3],v;
    if (!pcv->var) return 0;
    for (f=0;f<2;f++) {
        if ((idx=bds_ant_index(pcv,code[f],fallback))<0) return 0;
        for (j=0;j<3;j++) off[j]=pcv->off[idx][j]+del[j];
        v=0.0;
        if (pcvopt) v=pcv->dazi!=0.0?
            interpvar1(azel[0]*R2D,90.0-azel[1]*R2D,(pcv_t *)pcv,idx):
            interpvar0(0,90.0-azel[1]*R2D,pcv->var[idx]);
        dant[f]=-dot(off,e,3)+v;
    }
    return 1;
}
...
extern int antmodel_s_bds(const pcv_t *pcv, const uint8_t *code, double nadir, double *dant)
{
    ...
    if (!pcv->var) return 0;
    for (f=0;f<2;f++) {
        if ((idx=bds_ant_index(pcv,code[f],0))<0) return 0;
        dant[f]=interpvar0(pcv->sat,nadir*R2D*5.0,pcv->var[idx]);
    }
    return 1;
}
