...
static char codepris[7][MAXFREQ][16]={  /* code priority for each freq-index */
    /* priorities by each constellation's code2idx() frequency slot */
...
/* BDS observation-band order used by code2idx(). The default retains this
 * fork's historical B1I/B3I/B2b layout. It can be changed from a processing
 * configuration with prcopt.bdsfreqs (for example B1I,B3I,B1C). */
static char bdsfreqs[MAXFREQ]={'2','6','7','5','8','1'};

/* configure BDS frequency slots ---------------------------------------------
 * args   : freqs I comma/space separated bands: B1I,B1C,B2B,B2A,B3I,B2AB
 *               NULL or empty restores the historical default
 * return : number of explicitly selected bands (0:error)
 * notes  : unselected bands remain available as extended observations after
 *          the selected bands. The first NFREQ slots are solution slots.
 *---------------------------------------------------------------------------*/
extern int setbdsfreqs(const char *freqs)
{
    static const char default_order[MAXFREQ]={'2','6','7','5','8','1'};
    static const char all_bands[MAXFREQ]={'2','6','7','5','8','1'};
    static const char *band_pri[9]={"","DPX","IQX","","","DPX",
                                    "IQXA","IQXDPZ","DPX"};
    char buff[128],token[16],order[MAXFREQ],band=0,*p,*q;
    int i,j,n=0,selected,len;

    if (!freqs||!*freqs) {
        memcpy(order,default_order,sizeof(order));
        selected=NFREQ<MAXFREQ?NFREQ:MAXFREQ;
    }
    else {
        strncpy(buff,freqs,sizeof(buff)-1); buff[sizeof(buff)-1]='\0';
        for (p=buff;*p;) {
            while (*p==','||*p==';'||isspace((unsigned char)*p)) p++;
            if (!*p) break;
            for (q=p;*q&&*q!=','&&*q!=';'&&!isspace((unsigned char)*q);q++);
            len=(int)(q-p); if (len>=(int)sizeof(token)) return 0;
            for (i=0;i<len;i++) token[i]=(char)toupper((unsigned char)p[i]);
            token[len]='\0'; p=q;

            if      (!strcmp(token,"B1I" )) band='2';
            else if (!strcmp(token,"B1C" )) band='1';
            else if (!strcmp(token,"B2B" )||!strcmp(token,"B2I")) band='7';
            else if (!strcmp(token,"B2A" )) band='5';
            else if (!strcmp(token,"B3I" )||!strcmp(token,"B3")) band='6';
            else if (!strcmp(token,"B2AB")) band='8';
            else return 0;
            for (i=0;i<n;i++) if (order[i]==band) return 0;
            if (n>=MAXFREQ) return 0;
            order[n++]=band;
        }
        if (n<=0||n>NFREQ) return 0;
        selected=n;
        for (i=0;i<MAXFREQ;i++) {
            for (j=0;j<n;j++) if (order[j]==all_bands[i]) break;
            if (j==n) order[n++]=all_bands[i];
        }
    }
    memcpy(bdsfreqs,order,sizeof(bdsfreqs));
    for (i=0;i<MAXFREQ;i++) {
        int k=bdsfreqs[i]-'0';
        strcpy(codepris[5][i],0<=k&&k<=8?band_pri[k]:"");
    }
    return selected;
}
...
static int code2freq_BDS(uint8_t code, double *freq)
{
    ...
    switch (band) {
        case '1': *freq=FREQL1;    break; /* B1C */
        case '2': *freq=FREQ1_CMP; break; /* B1I */
        case '7': *freq=FREQ2_CMP; break; /* B2b */
        case '5': *freq=FREQL5;    break; /* B2a */
        case '6': *freq=FREQ3_CMP; break; /* B3I */
        case '8': *freq=FREQE5ab;  break; /* B2ab */
        default: return -1;
    }
    for (i=0;i<MAXFREQ;i++) if (bdsfreqs[i]==band) return i;
    return -1;
}
...
*            QZSS      L1    L2    L5    L6     -
*            SBAS      L1     -    L5     -     -
 *            BDS       configurable by prcopt.bdsfreqs
*            NavIC     L5     S     -     -     -
*-----------------------------------------------------------------------------*/
