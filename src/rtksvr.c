...
static void update_B2bssr(rtksvr_t *svr, int index)
{
    ...
    raw_t *raw = &svr->raw[index];  // Use pointer directly for efficiency
    nav_t *nav = &svr->nav;         // Point to global navigation data
    ...
    for (i = 0; i < MAXSAT; i++) {
        ...
        /* Skip satellites without update flag */
        ...
        if (raw->num_PPPB2BINF02 != 0) {  // Orbit message
            /* Check for duplicate orbit data */
            ...
            /* Check consistency between B2b IODN and Eph IODC */
            if(i>0){
        ...
        else if (raw->num_PPPB2BINF03 != 0) {  // CodeBias message
            /* Check for duplicate CodeBias data */
            ...
            raw->nav.B2bssr[i].udi[1] = timediff(raw->nav.B2bssr[i].t0[1],
                                                nav->B2bssr[i].t0[1]);
            // Set to 0 directly if the number of received messages is less than 2
            if(raw->nav.B2bssr[i].udi[1] > 86400) raw->nav.B2bssr[i].udi[1] = 0;

            /* Check IODC consistency */
            if (raw->nav.B2bssr[i].iodcorr[0] != raw->nav.B2bssr[i].iodcorr[1]) {
                trace(22,"ERROR: Orbit IODC mismatch (%d vs %d)\n",
                      raw->nav.B2bssr[i].iodcorr[0], 
                      raw->nav.B2bssr[i].iodcorr[1]);
            ...
            /* Check for duplicate Clock data */
        ...
        svr->nmsg[index][8]++; // Increment B2bssr counter
...
/*            if (!strstr(svr->rtk.opt.pppopt,"-DIS_FCB")) {
        ...
        //         printf("CPU OVERLOAD! Current cycle time: %d ms, exceeded allowed cycle time: %d ms\n", 
        //    (int)(tickget() - tick), svr->cycle);
...
/*			if (svr->rtk.dtr == 0 && svr->rtk.sol.stat == SOLQ_PPP){
				svr->rtk.dtr = svr->rtk.sol.dtr[0] + svr->rtk.sol.dtr[i];
			}

			if (sol1.dtr[i] == 0.0 && svr->rtk.sol.stat == SOLQ_PPP && svr->pppnum>pppnum){
				sol1 = svr->rtk.sol;
			}
			else if (svr->rtk.sol.stat == SOLQ_PPP && sol1.dtr[0] != 0.0){
				dtt = fabs(svr->rtk.sol.dtr[0] * 1E+09 / CLIGHT - sol1.dtr[0] * 1E+09 / CLIGHT);
				sol1.dtr[0] = svr->rtk.sol.dtr[0];
			}


			time2epoch(svr->rtk.sol.time, ep);
			y1 = ep[0];
			mon1 = ep[1];
			day1 = ep[2];
			h1 = ep[3] + ep[4] / 60.0 + ep[5] / 3600.0;

//			doy1 = julday(y1, mon1, day1, h1) - mjd2021;
			doy1 = 0.;

			sprintf(filename, "PPP%04d.txt", doy1);
			fp = fopen(filename, "a");
			fprintf(fp, "%04.0f-%02.0f-%02.0f %02.0f:%02.0f:%02.0f %3d  %15.3f  %15.3f  %15.3f %15.3f dtt=%6.3f\n",
				ep[0], ep[1], ep[2], ep[3], ep[4], ep[5], svr->rtk.sol.stat, (svr->rtk.sol.dtr[0] + svr->rtk.sol.dtr[3]) * 1E+09 / CLIGHT, svr->rtk.sol.rr[0],
				svr->rtk.sol.rr[1], svr->rtk.sol.rr[2], dtt);
			fclose(fp);
			
			fprintf(stderr, "%3d%15.3f%15.3f%15.3f%15.3f dtt=%6.3f\n", svr->rtk.sol.stat, (svr->rtk.sol.dtr[0] + svr->rtk.sol.dtr[3]) * 1E+09 / CLIGHT, svr->rtk.sol.rr[0],
				svr->rtk.sol.rr[1], svr->rtk.sol.rr[2], dtt);
*/
